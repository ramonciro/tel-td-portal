...
export default function PresencasPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.message || "Erro ao salvar presença");
      }
  ...
  return (
    <PortalShell title="Presenças" subtitle="Cadastro, edição e exclusão de presenças.">
      ...
      <div style={grid}>
        <div style={panel}>
          ...
          <form onSubmit={handleSubmit} style={formGrid}>
            ...
            <label style={label}>
              <input type="checkbox" name="presente" checked={form.presente} onChange={handleChange} />
              Presente
            </label>
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td style={thtd}>{item.presente ? "Sim" : "Não"}</td>
                    <td style={thtd}>
                ...
                {items.length === 0 ? (
                  <tr><td style={thtd} colSpan="4">Nenhuma presença cadastrada.</td></tr>
                ) : null}
              </tbody>
            </table>
          </div>
