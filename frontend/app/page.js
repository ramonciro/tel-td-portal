...
export default function EvolucaoColaboradorPage() {
  const [usuarios, setUsuarios] = useState([]); const [treinamentos, setTreinamentos] = useState([]); const [presencas, setPresencas] = useState([]); const [avaliacoes, setAvaliacoes] = useState([]); const [erro, setErro] = useState("")
  useEffect(() => {
    Promise.all([apiFetch("/usuarios"), apiFetch("/treinamentos"), apiFetch("/presencas"), apiFetch("/avaliacoes")])
      .then(([u,t,p,a]) => { setUsuarios(u); setTreinamentos(t); setPresencas(p); setAvaliacoes(a) })
  ...
  return (
    <PortalShell title="Evolução do Colaborador" subtitle="Painel com dados reais do portal">
      {erro ? <div style={errorBox}>{erro}</div> : null}
      <div style={grid}>
        {cards.map((c) => <div key={c.nome} style={card}><div style={{ fontSize: 18, fontWeight: 700 }}>{c.nome}</div><div style={{ color: "#64748b", marginTop: 4 }}>{c.perfil} • {c.cliente || "-"}</div><div style={{ marginTop: 12 }}>Treinamentos: <strong>{c.treinamentos}</strong></div><div>Assiduidade: <strong>{c.assiduidade}%</strong></div><div>Nota média: <strong>{c.nota}</strong></div></div>)}
