...
export default function ChamadaTurmaPage() {
  ...
  const todosFiltradosSelecionados = useMemo(() => {
    ...
    return registrosFiltrados.every((item) => selecionados[item.id || item.treinando_nome]);
  }, [registrosFiltrados, selecionados]);

  function atualizarRegistro(chave, campo, valor) {
    setRegistros((prev) =>
      prev.map((item) => ((item.id || item.treinando_nome) === chave ? { ...item, [campo]: valor } : item))
    );
  }
      ...
      <div style={sectionCard}>
        ...
        <div style={bulkBar}>
          <label style={checkWrap}>
            <input type="checkbox" checked={todosFiltradosSelecionados} onChange={alternarSelecaoTodosFiltrados} />
            <span>Selecionar todos os exibidos</span>
          </label>
          <div style={bulkInfo}>{totalSelecionados} selecionado(s)</div>
          <div style={bulkActions}>
            <button style={btnGhost} onClick={() => aplicarStatusSelecionados("presente")} disabled={!totalSelecionados}>Presente</button>
            <button style={btnGhost} onClick={() => aplicarStatusSelecionados("ausente")} disabled={!totalSelecionados}>Ausente</button>
            <button style={btnGhost} onClick={() => aplicarStatusSelecionados("justificado")} disabled={!totalSelecionados}>Justificado</button>
            <button style={btnGhost} onClick={() => aplicarStatusSelecionados("pendente")} disabled={!totalSelecionados}>Pendente</button>
            <button style={btnGhostDanger} onClick={limparSelecao} disabled={!totalSelecionados}>Limpar seleção</button>
          </div>
        </div>
            ...
            <tbody>
              {registrosFiltrados.map((item) => {
                ...
                return (
                  <tr key={chave}>
                    <td style={tdSmall}>
                      <input type="checkbox" checked={!!selecionados[chave]} onChange={() => alternarSelecao(chave)} />
                    </td>
                    ...
                    <td style={td}>
                      <select
                        ...
                        onChange={(e) => atualizarRegistro(chave, "status", e.target.value)}
                      >
                    ...
                    <td style={td}>
                      <input
                        ...
                        onChange={(e) => atualizarRegistro(chave, "justificativa", e.target.value)}
...
const heroGrid = { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12, marginTop: 18 };
const infoCard = { background: "rgba(255,255,255,.12)", border: "1px solid rgba(255,255,255,.16)", borderRadius: 18, padding: 16 };
const infoLabel = { fontSize: 12, textTransform: "uppercase", letterSpacing: ".03em", opacity: .8, fontWeight: 800 };
const infoValue = { marginTop: 6, fontSize: 18, fontWeight: 800, lineHeight: 1.3 };
const errorBox = { marginTop: 16, background: "#fef2f2", color: "#b91c1c", border: "1px solid #fecaca", borderRadius: 14, padding: 14, fontWeight: 700 };
const successBox = { marginTop: 16, background: "#ecfdf5", color: "#166534", border: "1px solid #bbf7d0", borderRadius: 14, padding: 14, fontWeight: 700 };
const statsGrid = { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12, marginTop: 18 };
const statCard = { background: "#fff", border: "1px solid #dbe4f0", borderRadius: 18, padding: 16, boxShadow: "0 10px 24px rgba(15,23,42,.04)" };
const statTitle = { color: "#64748b", fontWeight: 700, fontSize: 14 };
const statValue = { color: "#0f172a", fontWeight: 900, fontSize: 28, marginTop: 4 };
const sectionCard = { background: "#fff", border: "1px solid #dbe4f0", borderRadius: 24, padding: 20, marginTop: 18, boxShadow: "0 12px 28px rgba(15,23,42,.04)" };
const toolbar = { display: "flex", gap: 12, justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap" };
const sectionTitle = { margin: 0, fontSize: 28, color: "#0f172a" };
const sectionSubtitle = { margin: "6px 0 0", color: "#64748b" };
const toolbarActions = { display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" };
const bulkBar = { marginTop: 16, marginBottom: 16, padding: 14, borderRadius: 16, background: "#eff6ff", border: "1px solid #bfdbfe", display: "flex", gap: 12, justifyContent: "space-between", alignItems: "center", flexWrap: "wrap" };
const checkWrap = { display: "flex", gap: 8, alignItems: "center", fontWeight: 700, color: "#1e3a8a" };
const bulkInfo = { color: "#334155", fontWeight: 700 };
const bulkActions = { display: "flex", gap: 8, flexWrap: "wrap" };
const btnPrimary = { background: "linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%)", color: "#fff", border: 0, borderRadius: 12, padding: "11px 16px", fontWeight: 800, cursor: "pointer" };
const btnGhost = { background: "#fff", color: "#1d4ed8", border: "1px solid #bfdbfe", borderRadius: 10, padding: "9px 12px", fontWeight: 800, cursor: "pointer" };
const btnGhostDanger = { background: "#fff", color: "#b91c1c", border: "1px solid #fecaca", borderRadius: 10, padding: "9px 12px", fontWeight: 800, cursor: "pointer" };
const table = { width: "100%", borderCollapse: "collapse", minWidth: 740 };
const th = { textAlign: "left", padding: "12px 14px", fontSize: 13, color: "#475569", borderBottom: "1px solid #e2e8f0" };
const thSmall = { ...th, width: 42 };
const td = { padding: "12px 14px", borderBottom: "1px solid #edf2f7", color: "#0f172a", verticalAlign: "top" };
const tdSmall = { ...td, width: 42 };
const field = { width: "100%", minWidth: 220, borderRadius: 12, border: "1px solid #cbd5e1", background: "#fff", padding: "10px 12px", outline: "none" };
const select = { width: 180, borderRadius: 12, border: "1px solid #cbd5e1", background: "#fff", padding: "10px 12px", outline: "none" };
