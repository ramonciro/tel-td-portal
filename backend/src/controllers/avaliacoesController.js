...
export async function listAvaliacoes(req, res) {
  try {
    const [rows] = await pool.query(`
      SELECT id, treinamento_id, nota_nps, nota_qualidade, COALESCE(nota_prova, 0) AS nota_prova, comentario
      FROM avaliacoes
      ORDER BY id DESC
    `);
    res.json(rows);
...
export async function createAvaliacao(req, res) {
  try {
    const { treinamento_id, nota_nps, nota_qualidade, nota_prova, comentario } = req.body || {};
    ...
    const [result] = await pool.query(
      `INSERT INTO avaliacoes (treinamento_id, nota_nps, nota_qualidade, nota_prova, comentario)
       VALUES (?, ?, ?, ?, ?)`,
      [treinamento_id, nota_nps, nota_qualidade, nota_prova || 0, comentario || null]
    );

    res.status(201).json({ ok: true, id: result.insertId });
  } catch (error) {
    res.status(500).json({ ok: false, message: "Erro ao registrar avaliação", error: error.message });
  }
}
...
export async function updateAvaliacao(req, res) {
  try {
    ...
    await pool.query(
      `UPDATE avaliacoes
       SET treinamento_id = ?, nota_nps = ?, nota_qualidade = ?, nota_prova = ?, comentario = ?
       WHERE id = ?`,
      [treinamento_id, nota_nps, nota_qualidade, nota_prova || 0, comentario || null, id]
    );
