...
app.use("/api/clientes", createCrudRouter({
  table: "clientes",
  fields: ["nome","segmento","status","gestor","descricao"],
  orderBy: "nome ASC"
}))

app.use("/api/usuarios", createCrudRouter({
  table: "usuarios",
  fields: ["nome","email","senha","perfil","cliente","ativo"]
}))

app.use("/api/treinamentos", createCrudRouter({
  table: "treinamentos",
  fields: ["tema","cliente","instrutor","carga_horaria"]
}))

app.use("/api/presencas", createCrudRouter({
  table: "presencas",
  fields: ["treinamento_id","treinando_nome","status","justificativa"]
}))

app.use("/api/avaliacoes", createCrudRouter({
  table: "avaliacoes",
...
app.use("/api/biblioteca", createCrudRouter({
  table: "biblioteca_conteudos",
  fields: ["titulo","tipo","cliente","link_arquivo","descricao"],
  orderBy: "id DESC"
}))

app.use("/api/trilhas", createCrudRouter({
